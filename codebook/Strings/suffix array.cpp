/**
 * O (n log^2 (n))
 * See http://web.stanford.edu/class/cs97si/suffix-array.pdf for
 * reference
 * */
...
struct SuffixArray {
 ...
 SuffixArray(const string &s)
     : N(s.length()), s(s), P(1, vector<int>(N, 0)), M(N) {
  ...
  for (int skip = 1, level = 1; skip < N; skip *= 2, level++) {
   ...
   for (int i = 0; i < N; ++i)
    P[level][M[i].p] =
        (i > 0 and M[i].a == M[i - 1].a and M[i].b == M[i - 1].b)
            ? P[level][M[i - 1].p]
            : i;
  }
 }
 ...
 vector<int> getSuffixArray() {
  ...
  for (int i = 0; i < rank.size(); ++i) sa[i] = inv[i].second;
  return sa;
 }
