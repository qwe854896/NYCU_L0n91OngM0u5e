...
struct wavelet {
 ...
 wavelet(int a, int b, int c)
     : l(a), r(b), m(c), left(NULL), right(NULL) {}
};
...
int kth(wavelet *node, int to, int k) {
 ...
 if (k < c) return kth(node->left, c - 1, k);
 return kth(node->right, node->map_right[to] - 1, k - c);
}

int pos_kth_ocurrence(wavelet *node, int val, int k) {
 // returns the position on the original array of the kth ocurrence of
 // the value "val"
 if (!node) return -1;

 if (node->l == node->r) {
  if (int(node->ori.size()) <= k) return -1;
  return node->ori[k];
 }

 if (val <= node->m) return pos_kth_ocurrence(node->left, val, k);
