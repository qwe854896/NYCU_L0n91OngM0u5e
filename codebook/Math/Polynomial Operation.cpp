...
struct Poly : vector<LL> {        // coefficients in [0, P)
 ...
 vector<LL> _eval(const vector<LL> &x, const vector<Poly> &up) const {
  ...
  /* down[1] = Poly(up[1]).irev().isz(n()).Inv().irev()._tmul(_n,
  *this); fi(2, _n * 2) down[i] = up[i ^ 1]._tmul(up[i].n() - 1,
  down[i / 2]); */
  vector<LL> y(_n);
 ...
 Poly Pow(const string &K) const {
  ...
  LL x0 = X[0];
  return X.imul(ntt.minv(x0))
      .Ln()
      .imul(nk)
      .Exp()
      .imul(ntt.mpow(x0, nk2))
      .irev()
      .isz(n())
      .irev();
 }
 static LL LinearRecursion(const vector<LL> &a,
                           const vector<LL> &coef,
                           LL n) {  // a_n = \sum c_j a_(n-j)
...
template <>
decltype(Poly_t::ntt) Poly_t::ntt = {};
